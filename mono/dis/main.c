...
dis_field_list (MonoImage *m, guint32 start, guint32 end, MonoGenericContainer *container)
{
    ...
    for (i = start; i < end; i++){
        ...
        if (cols [MONO_FIELD_FLAGS] & FIELD_ATTRIBUTE_HAS_FIELD_MARSHAL) {
            ...
            tp = mono_metadata_get_marshal_info (m, idx, TRUE);
        ...
        mono_metadata_field_info (m, idx, &field_offset, NULL, NULL);
        if (field_offset != -1)
        ...
        if (cols [MONO_FIELD_FLAGS] & FIELD_ATTRIBUTE_HAS_DEFAULT){
            ...
            if ((crow = mono_metadata_get_constant_index (m, MONO_TOKEN_FIELD_DEF | (idx + 1), 0))) {
        ...
        g_free (sig);
        g_free (esname);
        dump_cattrs (m, MONO_TOKEN_FIELD_DEF | (idx + 1), "    ");
    }
}
...
dump_cattrs_for_method_params (MonoImage *m, guint32 midx, MonoMethodSignature *sig) {
    ...
    for (i = param_index; i < lastp; ++i) {
        ...
        list = dis_get_custom_attrs (m, MONO_TOKEN_PARAM_DEF | idx);

        mono_metadata_decode_row (paramt, idx-1, param_cols, MONO_PARAM_SIZE);
        if (!(param_cols[MONO_PARAM_FLAGS] & PARAM_ATTRIBUTE_HAS_DEFAULT)) {
            ...
            if ((crow = mono_metadata_get_constant_index(m, MONO_TOKEN_PARAM_DEF | idx, 0))) {
                guint32 const_cols [MONO_CONSTANT_SIZE];
                mono_metadata_decode_row( &m->tables[MONO_TABLE_CONSTANT], crow-1, const_cols, MONO_CONSTANT_SIZE);
                lit = get_constant (m, (MonoTypeEnum)const_cols [MONO_CONSTANT_TYPE], const_cols [MONO_CONSTANT_VALUE]);
            }
...
dis_method_list (const char *klass_name, MonoImage *m, guint32 start, guint32 end, MonoGenericContainer *type_container)
{
    ...
    for (i = start; i < end; i++){
        ...
        // Modified by zx
        idx = i;
        if (m->is_rgdll && m->tables [MONO_TABLE_METHOD_POINTER].rows)
            idx = mono_metadata_decode_row_col (&m->tables [MONO_TABLE_METHOD_POINTER], idx, MONO_METHOD_POINTER_METHOD) - 1;

        if (!should_include_method (idx + 1))
            continue;
        mono_metadata_decode_row (t, idx, cols, MONO_METHOD_SIZE);
        ...
        container = mono_metadata_load_generic_params (m, MONO_TOKEN_METHOD_DEF | (idx + 1), type_container, NULL);
        if (container) {
            ERROR_DECL (error);
            mono_metadata_load_generic_param_constraints_checked (m, MONO_TOKEN_METHOD_DEF | (idx + 1), container, error);
            g_assert (is_ok (error)); /*FIXME don't swallow the error message*/
        } else {
            container = type_container;
        }

        ms = mono_metadata_parse_method_signature_full (m, container, idx + 1, sig, &sig, error);
        if (ms != NULL){
            sig_str = dis_stringify_method_signature (m, ms, idx + 1, container, FALSE);
        ...
        token = MONO_TOKEN_METHOD_DEF | (idx + 1);
        
        fprintf (output, "    {\n");
        dump_cattrs (m, token, "        ");
        dump_cattrs_for_type_params (m, MONO_TOKEN_METHOD_DEF | (idx + 1), "        ");
        dump_cattrs_for_method_params (m, idx, ms);

        fprintf (output, "        // Method begins at RVA 0x%x\n", cols [MONO_METHOD_RVA]);
        dump_declarative_security (m, OBJECT_TYPE_METHODDEF, idx + 1, "        ");
...
{
    guint start, end, i, idx;
    start = mono_metadata_properties_from_typedef (m, typedef_row, &end);

    for (i = start; i < end; ++i) {
        idx = i;
        if (m->is_rgdll && m->tables [MONO_TABLE_PROPERTY_POINTER].rows)
            idx = mono_metadata_decode_row_col (&m->tables [MONO_TABLE_PROPERTY_POINTER], idx, MONO_PROPERTY_POINTER_PROPERTY) - 1;
        char *sig = dis_property_signature (m, idx, container);
        fprintf (output, "\t.property %s\n\t{\n", sig);
        dump_cattrs (m, MONO_TOKEN_PROPERTY | (idx + 1), "\t\t");
        dis_property_methods (m, idx, container);
        fprintf (output, "\t}\n");
        g_free (sig);
    }
}
...
dis_event_list (MonoImage *m, guint32 typedef_row, MonoGenericContainer *container)
{
    ...
    for (i = start; i < end; ++i) {
        idx = i;
        if (m->is_rgdll && m->tables [MONO_TABLE_EVENT_POINTER].rows)
            idx = mono_metadata_decode_row_col (&m->tables [MONO_TABLE_EVENT_POINTER], idx, MONO_EVENT_POINTER_EVENT) - 1;
        char *sig = dis_event_signature (m, idx, container);
        fprintf (output, "\t.event %s\n\t{\n", sig);
        dump_cattrs (m, MONO_TOKEN_EVENT | (idx + 1), "\t\t");
        dis_event_methods (m, idx, container);
        fprintf (output, "\t}\n");
        g_free (sig);
    }
}
