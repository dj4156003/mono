...
dump_table_field (MonoImage *m)
{
    ...
    MonoTableInfo *tp = &m->tables [MONO_TABLE_FIELD_POINTER];
    ...
    int i, current_type, offset_row, rva_row, idx;
    guint32 first_m, last_m;
    
    gboolean usePointerMap = m->is_rgdll && tp->rows > 0;
    int tcount = usePointerMap ? tp->rows : t->rows;
    ...
    for (i = 1; i <= tcount; i++){
        ...
        if (usePointerMap)
            idx = mono_metadata_decode_row_col (tp, idx - 1, MONO_FIELD_POINTER_FIELD);

        mono_metadata_decode_row (t, idx - 1, cols, MONO_FIELD_SIZE);
        ...
        g_free (sig);
        g_free (flags);
        if (offset_row <= fl->rows && (mono_metadata_decode_row_col (fl, offset_row - 1, MONO_FIELD_LAYOUT_FIELD) == idx)) {
            fprintf (output, "\texplicit offset: %d\n", mono_metadata_decode_row_col (fl, offset_row - 1, MONO_FIELD_LAYOUT_OFFSET));
            offset_row ++;
        }
        if (rva_row <= rva->rows && (mono_metadata_decode_row_col (rva, rva_row - 1, MONO_FIELD_RVA_FIELD) == idx)) {
...
dump_table_method (MonoImage *m)
{
    ...
    MonoTableInfo *tp = &m->tables [MONO_TABLE_METHOD_POINTER];
    MonoTableInfo *td = &m->tables [MONO_TABLE_TYPEDEF];
    int i, current_type, idx;
    guint32 first_m, last_m;
    ...
    for (i = 1; i <= tcount; i++){
        ...
        if (usePointerMap)
            idx = mono_metadata_decode_row_col (tp, idx - 1, MONO_METHOD_POINTER_METHOD);

        method_container = mono_metadata_load_generic_params (m, MONO_TOKEN_METHOD_DEF | idx, type_container, NULL);
        if (method_container) {
            mono_metadata_load_generic_param_constraints_checked (m, MONO_TOKEN_METHOD_DEF | idx, method_container, error);
            g_assert (is_ok (error)); /*FIXME don't swallow the error message*/
        }
        ...
        method = mono_metadata_parse_method_signature_full (m, method_container ? method_container : type_container, idx, sigblob, &sigblob, error);
        if (!is_ok (error)) {
            fprintf (output,"%d: failed to parse due to %s\n", idx, mono_error_get_message (error));
            mono_error_cleanup (error);
            continue;
        }
        
        g_assert (is_ok (error)); /*FIXME don't swallow the error message*/
        sig = dis_stringify_method_signature (m, method, idx, method_container ? method_container : type_container, FALSE);
        impl_flags = get_method_impl_flags (cols [MONO_METHOD_IMPLFLAGS]);
        fprintf (output, "%d: %s (param: %d impl_flags: %s)\n", idx, sig, cols [MONO_METHOD_PARAMLIST], impl_flags);
        g_free (sig);
        g_free (impl_flags);
