...
do
    ...
    python3 "$CURRENT_DIRECTORY"/osx.py configure --target=$target -j 2  --configuration="$CONFIGURATION" --configure-dir="$MONO_CONFIG_PATH" --install-dir="$MONO_INSTALL_PATH"
    python3 "$CURRENT_DIRECTORY"/osx.py make --target=$target -j 2  --configuration="$CONFIGURATION" --configure-dir="$MONO_CONFIG_PATH" --install-dir="$MONO_INSTALL_PATH"
done
