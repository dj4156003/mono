...
MONO_INSTALL_PATH="${CURRENT_DIRECTORY}/../install/ios_aot_mono_release/"
MONO_CONFIG_PATH="${CURRENT_DIRECTORY}/../config/ios_aot_mono_release/"
...
do
    ...
    python3 "$CURRENT_DIRECTORY"/ios.py configure --target="arm64" -j 2 --ios-version-min=$IOS_VERSION_MIN  --configuration="release" --configure-dir="$MONO_CONFIG_PATH" --install-dir="$MONO_INSTALL_PATH"
